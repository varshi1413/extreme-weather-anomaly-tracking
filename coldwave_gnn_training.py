r"""
Coldwave_GNN_Training.py

WeatherGPT - Coldwave-specific spatio-temporal GNN training (GPU-optimized).
...
Pipeline:
    ERA5 NetCDF (Single-Level + Pressure-Level)
        ...
        -> future Tmin regression + coldwave classification

Coldwave label:
    A future day is labelled as a coldwave when:
        future Tmin <= local training-period 5th percentile
        AND
        future Tmin - local training-period mean <= -2 degC

Important:
    This is a model-training pipeline. It does not by itself provide
    official IMD coldwave certification.

Recommended first test on CPU:
    python Coldwave_GNN_Training.py --era5 "C:\path\to\ERA5_INDIA" --epochs 2 --max-samples 30

Full training:
    python Coldwave_GNN_Training.py --era5 "C:\path\to\ERA5_INDIA" --epochs 50
...
class Config:
    ...
    # Output
    checkpoint_path: str = "coldwave_gnn.pt"
...
def _load_month_group(month, files, cfg):
    ...
    # Pressure-only 2021 files cannot provide the coldwave Tmin target.
...
def open_era5(files, cfg):
    ...
    if "tmin" not in ds_all.data_vars:
        raise ValueError("Daily Tmin could not be created because ERA5 t2m was unavailable.")
...
def daily_aggregate(ds, cfg):
    ...
    Single-Level examples:
        tmin, d2m, u10, v10, msl, sp, tp, skt, sst, tcc,
    ...
    for name in SINGLE_LEVEL_CANDIDATES:
        ...
        # Tmin is calculated from hourly/sub-daily 2-m temperature.
        if name == "t2m":
            daily_vars["tmin"] = da.resample(
                time="1D"
            ).min(skipna=True)
        elif name == "tp":
    ...
    if "tmin" not in daily_vars:
        raise ValueError(
            "Daily Tmin could not be created because ERA5 t2m was not available."
        )

    daily = xr.Dataset(daily_vars)

    # Remove dates for which the required Tmin target is unavailable.
...
def sample_daily_to_mesh(daily, geometry, cfg):
    ...
    feature_names = [
        ...
        if name not in ["tmin", "doy_sin", "doy_cos"]
    ]
    feature_names = ["tmin"] + feature_names + ["doy_sin", "doy_cos"]
...
def compute_coldwave_climatology(tmin, train_end):
    """
    ...
    """
    train_tmin = tmin[:train_end]

    mean = np.nanmean(train_tmin, axis=0)
    p05 = np.nanpercentile(train_tmin, 5, axis=0)

    return mean.astype(np.float32), p05.astype(np.float32)
...
class ColdwaveSequenceDataset(Dataset):
    def __init__(
        self,
        features,
        tmin,
    ...
    def __getitem__(self, idx):
        ...
        y_cold = self.cold_labels[
            i + self.window:i + self.window + self.horizon
        ]

        return (
            torch.from_numpy(x).float(),
            torch.from_numpy(y_tmin).float(),
            torch.from_numpy(y_cold).float(),
        )
...
# ============================================================
# 10. COLDWAVE MGN + TEMPORAL TRANSFORMER
# ============================================================

class ColdwaveGNN(nn.Module):
    def __init__(
        self,
        input_dim,
        num_nodes,
        ...
        self.coldwave_head = nn.Sequential(
            nn.Linear(temporal_dim, temporal_dim // 2),
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(temporal_dim // 2, horizon),
    ...
    def forward(
        ...
        x: [B, T, N, F]
        Returns:
            tmin_pred: [B, horizon, N]
            cold_logits: [B, horizon, N]
        """
        B, T, N, _ = x.shape
        ...
        tmin_pred = self.tmin_head(context).permute(0, 2, 1)
        cold_logits = self.coldwave_head(context).permute(0, 2, 1)

        return tmin_pred, cold_logits
...
def run_epoch(
    ...
    for x, y_tmin, y_cold in loader:
        x = x.to(device, non_blocking=True)
        y_tmin = y_tmin.to(device, non_blocking=True)
        y_cold = y_cold.to(device, non_blocking=True)
        ...
        if use_amp:
            with torch.cuda.amp.autocast(dtype=torch.float16):
                tmin_pred, cold_logits = model(
                    x, edge_index, edge_features, valid_mask
                )
                loss_reg = masked_mse(tmin_pred, y_tmin, valid_mask)
                loss_cls = masked_bce(
                    cold_logits, y_cold, valid_mask, pos_weight=pos_weight
                )
        ...
        all_tmin_pred.append(tmin_pred.detach())
        all_tmin_true.append(y_tmin.detach())
        all_cold_logits.append(cold_logits.detach())
        all_cold_true.append(y_cold.detach())

    if total_count == 0:
        return None

    tmin_pred = torch.cat(all_tmin_pred, dim=0)
    tmin_true = torch.cat(all_tmin_true, dim=0)
    cold_logits = torch.cat(all_cold_logits, dim=0)
    cold_true = torch.cat(all_cold_true, dim=0)

    mse, mae, rmse_c, mae_c = regression_metrics(
        tmin_pred, tmin_true, valid_mask, y_std
    )

    precision, recall, f1, tp, tn, fp, fn = classification_metrics(
        cold_logits, cold_true, valid_mask
    )
...
def prepare_data(cfg):
    ...
    tmin_index = feature_names.index("tmin")
    tmin = features[:, :, tmin_index].copy()
    ...
    clim_mean, clim_p05 = compute_coldwave_climatology(tmin, train_end)

    cold_labels = (
        (tmin <= clim_p05[None, :]) &
        ((tmin - clim_mean[None, :]) <= -2.0)
    ).astype(np.float32)
    ...
    train_y = tmin[:train_end, valid_mask]
    scaler_y.fit(train_y[:, :, None])
    del train_y

    tmin_scaled = scaler_y.transform(tmin[:, :, None])[:, :, 0].astype(np.float32)

    features_scaled[:, ~valid_mask, :] = 0.0
    tmin_scaled[:, ~valid_mask] = 0.0
    cold_labels[:, ~valid_mask] = 0.0
    ...
    if cfg.max_samples > 0:
        ...
        tmin_scaled = tmin_scaled[:max_needed]
        cold_labels = cold_labels[:max_needed]
    ...
    return {
        ...
        "tmin": tmin_scaled,
        "cold_labels": cold_labels.astype(np.float32),
        "train_end": train_end,
        "val_end": val_end,
        "mesh_lat": geometry["mesh_lat"],
        "mesh_lon": geometry["mesh_lon"],
...
def save_checkpoint(
    ...
    checkpoint = {
        ...
        "coldwave_climatology_mean": data["clim_mean"],
        "coldwave_climatology_p05": data["clim_p05"],
    }
...
def train_coldwave(cfg):
    set_seed(cfg.seed)

    print("=" * 70)
    print("WEATHERGPT - COLDWAVE GNN TRAINING")
    print("=" * 70)
    ...
    train_ds = ColdwaveSequenceDataset(
        features,
        tmin,
        cold_labels,
        0,
    ...
    val_ds = ColdwaveSequenceDataset(
        features,
        tmin,
        cold_labels,
        train_end,
    ...
    test_ds = ColdwaveSequenceDataset(
        features,
        tmin,
        cold_labels,
        val_end,
    ...
    print("\n" + "=" * 70)
    print("[3/7] CREATING COLDWAVE GNN")
    print("=" * 70)
    ...
    # Counter class imbalance so the coldwave head is not rewarded for
    # predicting "no coldwave" everywhere.
    train_cold = data["cold_labels"][:train_end, data["valid_mask"]]
    positive = float(train_cold.sum())
    negative = float(train_cold.size - positive)
    ...
    print(
        f"Coldwave training labels: positive={int(positive):,} | "
    ...
    if test_metrics is not None:
        print("\nTest results:")
        print(
            f"  Tmin MAE : {test_metrics['mae_c']:.4f} °C"
        )
        print(
            f"  Tmin RMSE: {test_metrics['rmse_c']:.4f} °C"
        )
        print(
            f"  Cold Precision: {test_metrics['precision']:.4f}"
        )
        print(
            f"  Cold Recall   : {test_metrics['recall']:.4f}"
        )
        print(
            f"  Cold F1       : {test_metrics['f1']:.4f}"
        )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Number of future daily Tmin predictions.",
    )
